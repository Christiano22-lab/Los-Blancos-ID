                   ...
                   <?php foreach ($all_communities as $community): ?>
                       <div class="community-card" data-platform="<?php echo $community['platform']; ?>">
                           ...
                           <div class="card-footer">
                               ...
                               <button class="share-btn" onclick="shareCommunity('<?php echo htmlspecialchars($community['name']); ?>', '<?php echo htmlspecialchars($community['link']); ?>', '<?php echo htmlspecialchars($community['description']); ?>')">
...
function shareCommunity(name, link, description) {
    const shareText = `Bergabung dengan komunitas ${name} - Madridista Indonesia\n\n${description}`;
    const shareUrl = link;
    ...
    modalContent.innerHTML = `
        <h3 style="margin-bottom: 1.5rem; color: #002660;"><i class="fas fa-share-alt"></i> Share Komunitas</h3>
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1.5rem;">
            <button onclick="shareToWhatsApp('${encodeURIComponent(shareText)}', '${encodeURIComponent(shareUrl)}')" 
                    style="background: #25D366; color: white; border: none; padding: 0.75rem; border-radius: 8px; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 0.5rem;">
                <i class="fab fa-whatsapp"></i> WhatsApp
            </button>
            <button onclick="shareToFacebook('${encodeURIComponent(shareUrl)}')" 
                    style="background: #1877F2; color: white; border: none; padding: 0.75rem; border-radius: 8px; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 0.5rem;">
                <i class="fab fa-facebook-f"></i> Facebook
            </button>
            <button onclick="shareToTwitter('${encodeURIComponent(shareText)}', '${encodeURIComponent(shareUrl)}')" 
                    style="background: #1DA1F2; color: white; border: none; padding: 0.75rem; border-radius: 8px; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 0.5rem;">
                <i class="fab fa-twitter"></i> Twitter
            </button>
            <button onclick="shareToInstagram('${encodeURIComponent(shareText)}')" 
                    style="background: #E4405F; color: white; border: none; padding: 0.75rem; border-radius: 8px; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 0.5rem;">
                <i class="fab fa-instagram"></i> Instagram
            </button>
        </div>
        <div style="margin-bottom: 1rem;">
            <button onclick="copyToClipboard('${shareUrl}')" 
                    style="background: #6c757d; color: white; border: none; padding: 0.75rem 1.5rem; border-radius: 8px; cursor: pointer; width: 100%;">
                <i class="fas fa-copy"></i> Copy Link
            </button>
        </div>
        <button onclick="closeShareModal()" 
                style="background: #dc3545; color: white; border: none; padding: 0.5rem 1.5rem; border-radius: 8px; cursor: pointer;">
...
function shareToInstagram(text) {
    ...
    alert('Instagram: Text copied! Paste it in your Instagram story or post.');
    closeShareModal();
}
